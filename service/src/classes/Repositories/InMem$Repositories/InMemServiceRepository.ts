...
export class InMemServiceRepository extends InMemoryRepository<Service>
  ...
  override async save(service: Service): Promise<void> {
    let serviceIndex = this.inMemList.findIndex((e) =>
      service.getId() === e.getId()
    );
    if (serviceIndex < 0) {
      await this.add(service);
  ...
  override async removeById(serviceId: string): Promise<void> {
    try {
      await super.removeById(serviceId);
    } catch (e) {
